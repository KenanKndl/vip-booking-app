...
import { MapPin, Calendar, Clock, Users, ChevronRight, CheckCircle2, Baby, Check, ChevronLeft, Users2 } from "lucide-react";
import { useTranslations } from "next-intl";
import { useCurrencyStore } from "@/store/useCurrencyStore"; // YENİ EKLENDİ (Dosya yolunu kendi projene göre ayarla)

export function ReservationSection({ dbRoutes = [], exchangeRates }: { dbRoutes: any[], exchangeRates: { eurToTl: number, eurToUsd: number } }) {
    ...
    // YENİ: Para birimini artık lokal bir dropdown'dan değil, Global Store'dan (Üst Menüden) okuyoruz
    ...
    // Dinamik Fiyat Hesaplama Motoru
    ...
    const handleCompleteReservation = async () => {
        ...
        // Veritabanına yazılacak final fiyatı hesaplıyoruz
        const finalPrice = 
            displayCurrency === "TRY" ? selectedPricing.price * exchangeRates.eurToTl :
            displayCurrency === "USD" ? selectedPricing.price * exchangeRates.eurToUsd :
                        ...
                        {step === 2 && selectedRoute && (
                            <motion.div key="step2" variants={formVariants} initial="hidden" animate="visible" exit="exit" className="flex flex-col gap-10 relative z-10">
                                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-2">
                                    ...
                                    <button onClick={() => setStep(1)} className="text-xs font-semibold text-white/60 hover:text-white uppercase tracking-widest border border-white/10 px-5 py-2.5 rounded-full flex items-center gap-2 transition-colors hover:bg-white/5">
                                                ...
                                                <div className="p-8 flex flex-col justify-between">
                                                    ...
                                                    <div className="flex flex-col sm:flex-row items-center justify-between gap-6 pt-6 border-t border-white/10">
                                                        <div className="flex flex-col">
                                                            ...
                                                            {/* ANLIK HESAPLANMIŞ FİYAT */}
                                                            <p className="text-3xl font-bold text-white">{getFormattedPrice(pricing.price)}</p>
                                                        </div>
                                                        <button 
                                ...
                                <div className="grid gap-10 md:grid-cols-2">
                                    ...
                                    <div className="flex flex-col gap-5">
                                        ...
                                        <div className="p-5 rounded-2xl border border-white/10 bg-white/5">
                                            ...
                                            <p className="text-[#FACC15] text-2xl font-bold mt-4">{getFormattedPrice(selectedPricing.price)}</p>
                                        </div>
                                    </div>
                                </div>
