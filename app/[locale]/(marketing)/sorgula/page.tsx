...
const STATUS_CONFIG = {
  PENDING: { label: 'Onay Bekliyor', color: 'text-amber-500', bg: 'bg-amber-500/10', border: 'border-amber-500/20', icon: Clock },
  CONFIRMED: { label: 'Onaylandı', color: 'text-green-500', bg: 'bg-green-500/10', border: 'border-green-500/20', icon: CheckCircle2 },
  CANCELLED: { label: 'İptal Edildi', color: 'text-red-500', bg: 'bg-red-500/10', border: 'border-red-500/20', icon: XCircle },
  COMPLETED: { label: 'Tamamlandı', color: 'text-blue-500', bg: 'bg-blue-500/10', border: 'border-blue-500/20', icon: CheckCircle2 },
};
...
export default function PnrSorgulamaPage() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    // Backend'e gönderirken VIP takısını otomatik ekliyoruz
      ...
      <div className="mx-auto max-w-2xl w-full">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="text-3xl font-semibold tracking-tight text-white sm:text-5xl mb-4">
            Rezervasyon <span className="text-white/45">Sorgulama</span>
          </h1>
          <p className="text-sm text-white/45 max-w-md mx-auto">
            Sistemimizden size verilen PNR kodu ve e-posta adresiniz ile rezervasyonunuzun güncel durumunu anında öğrenebilirsiniz.
          </p>
        </div>
        ...
        >
          <div className="grid gap-5 sm:grid-cols-2 mb-6">
            <div className="space-y-2">
              <label className="pl-1 text-xs font-semibold uppercase tracking-[0.16em] text-white/45">PNR Kodu</label>
              
              {/* GÜNCELLENEN KISIM: Sabit VIP yazısı ve sadece rakam kabul eden input */}
              <div className="flex w-full overflow-hidden rounded-2xl border border-white/10 bg-black/20 transition duration-200 hover:border-white/15 focus-within:border-[#22D3EE]/50 focus-within:bg-black/30">
                ...
                <input 
                  type="text" 
                  value={pnr}
                  // Sadece rakam girişine izin veriyoruz ve harfleri anında siliyoruz
            ...
            <div className="space-y-2">
              <label className="pl-1 text-xs font-semibold uppercase tracking-[0.16em] text-white/45">E-posta Adresiniz</label>
              <input 
                type="email" 
                value={email}
          ...
          >
            {loading ? (
              ...
              <>
                Sorgula <Search className="w-4 h-4 group-hover:scale-110 transition-transform" />
              </>
            )}
          </button>
        </motion.form>

        {/* Sonuç Ekranı (Bilet Görünümü) */}
        <AnimatePresence>
          {result && (
            <motion.div 
            ...
            >
              ...
              <div className="p-6 sm:p-8 pb-10 flex flex-col items-center text-center">
                <p className="text-xs font-semibold uppercase tracking-widest text-white/45 mb-4">Transfer Durumu</p>
                <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full border ${STATUS_CONFIG[result.status].bg} ${STATUS_CONFIG[result.status].border} ${STATUS_CONFIG[result.status].color}`}>
                  ...
                  <span className="font-bold text-sm tracking-wide">{STATUS_CONFIG[result.status].label}</span>
                </div>
              ...
              <div className="p-6 sm:p-8 bg-white/[0.02] grid gap-6 sm:grid-cols-2">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <p className="text-[10px] font-semibold uppercase tracking-wider text-white/45">Tarih & Saat</p>
                    <p className="text-sm font-medium text-white mt-1">
                      {new Date(result.pickupDateTime).toLocaleString('tr-TR', { 
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <p className="text-[10px] font-semibold uppercase tracking-wider text-white/45">Toplam Tutar</p>
                ...
                <div className="flex items-start gap-4 sm:col-span-2 bg-black/20 p-4 rounded-2xl border border-white/5">
                  ...
                  <div className="min-w-0">
                    <p className="text-[10px] font-semibold uppercase tracking-wider text-white/45 mb-1.5 flex items-center gap-2">
                      Transfer Rotası <span className="px-1.5 py-0.5 rounded text-[9px] bg-white/10 text-white/70">{result.tripType === 'ROUND_TRIP' ? 'Gidiş-Dönüş' : 'Tek Yön'}</span>
                    </p>
